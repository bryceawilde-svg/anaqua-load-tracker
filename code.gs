...
function buildVisionContent(imageB64, mime) {
  if (mime === 'application/pdf') {
    return { type: 'document', source: { type: 'base64', media_type: 'application/pdf', data: imageB64 } };
  }
  return { type: 'image', source: { type: 'base64', media_type: mime || 'image/jpeg', data: imageB64 } };
}
...
function captureFieldTicket(payload) {
  ...
  const f = JSON.parse(callClaudeSonnet([
    buildVisionContent(imageB64, mime),
...
function matchDeliveredTicket(payload) {
  ...
  const prompt = 'You are reading a DELIVERED/SCALE TICKET from a grain elevator or buyer in Texas. ' +
    ...
    'For GSI International tickets (look for "GSI International" or "RGV- GSI International" in the header, and the title "UNITED STATES WAREHOUSE ACT GRAIN INSPECTION AND WEIGHT CERTIFICATE"): buyer = "GSI International". ticket_number = the alphanumeric ID in the top-right box (e.g. "PG3-00010"), also repeated at the bottom as "Single ticket: #..." — capture it exactly as printed, including letters and dashes, do NOT strip letters or convert it to a plain integer. field_ticket_ref = the number next to "Reference:" in the Commodity section (e.g. "5133"). field_lot = the second line under the "Customer" box, directly below the "Anaqua Farms" line (e.g. "527 MEADE SOUTH") — do NOT use the mailing address line below it. driver = the name in the "Transport (Truck)" section, also repeated at the bottom next to "Driver:". crop = the first line under "Commodity" (e.g. "Sorghum"). gross_weight = the value under "Gross (lb)". tare_weight = the value under "Tare TPT (lb)". net_weight = the value under "Net (lb)" or "Net weight (lb)" in the TOTAL WEIGHT box. moisture = the value labeled "MS" under "Characteristics Grades of Grain". bushel_weight = the value labeled "TW+" under "Characteristics Grades of Grain". ' +
